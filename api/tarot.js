export default async function handler(req, res) {
  ...
  const nameCall = userName ? `访客的名字叫“${userName}”，请在文中自然地称呼TA。` : "称呼访客为亲爱的朋友。";
...
绝对不要解释牌意，直接输出这段诗意的话。不要任何排版标签。`;
  } 
  // 模式 B：超长专业解盘模式（流式输出）
  else {
    const tone = isNight 
      ? "现在是深夜，你的语气要变得极度轻柔、催眠、带有深深的疗愈感和包容，安抚深夜容易焦虑的灵魂。" 
      : "你是一位极具洞察力、从业二十年的顶尖塔罗大师。";
    
    systemRole = tone + "你必须输出极其专业、篇幅在 1000 - 1500 字的深度解析。";
...
1. 🔮 【命运总览】：用一句充满哲理的金句定下基调，然后一针见血地直接回答TA的问题（是/否/吉/凶）。
2. 🌟 【灵魂拆解】：不要单调地报菜名！将这几张牌的内在逻辑交织在一起，像看电影一样讲出TA身上正在发生的故事。过去埋下了什么因？现在卡在哪里？对方真正的潜台词是什么？
3. 💡 【破局之眼】：在这几张牌中挑出最关键的一张，极其深刻地指出问题的死穴所在。
4. ✨ 【凡尘指南】：给出 3 条极度具体的现实行动建议。第一条关于心态，第二条关于行动，第三条是送给TA的心理护身符。

排版铁律：
1. 严格使用 HTML 标签排版（<h4>, <p>, <strong>, <ul>）。
2. 绝对禁止输出 markdown 的 \`\`\`html！绝对禁止输出 <style> 标签或颜色代码！`;
  }
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: '大模型连接失败' });
    }
