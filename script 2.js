...
async function loadDeck() {
  try {
    ...
    console.error(error);
    // 可以在这里添加一些用户提示，比如在页面上显示错误信息
    updateStatus("错误：无法加载卡牌数据，部分功能可能无法使用。");
  }
}

window.onload = async function() {
  await loadDeck(); // 等待卡牌数据加载完成
  isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  checkNightMode(); 
  initStarfield(); 
  renderSpread(); 
  loadHistory(); 
  updateStatus("请选择占卜方式：直觉速取或深度问卜");
  
  const introStr = isNightMode ? "亲爱的旅人，夜深了。\n闭上眼，告诉我你心中的困惑..." : "亲爱的旅人，欢迎来到命运星盘。\n深呼吸，告诉我你最想问的是什么...";
  let i = 0;
  function typeIntro() {
    if(i < introStr.length) {
      document.getElementById('introText').innerText += introStr.charAt(i);
      i++;
...
function loadHistory() {
  ...
  if (raw) {
    try { 
      historyRecords = JSON.parse(raw).slice(0, 6); 
    } catch (e) { 
      historyRecords = []; 
    }
  }
...
function addHistoryRecord(record) {
  ...
  historyRecords = historyRecords.slice(0, 6); 
  saveHistory(); 
  renderHistory();
}
...
function forcePlayMusic() { 
  if (!isMusicPlaying) { 
    ...
    if (p !== undefined) { 
      p.then(_ => { 
        isMusicPlaying = true; 
        document.getElementById("musicToggle").innerText = "🔇 静音"; 
      }).catch(e => {}); 
    } 
  } 
}
...
function mobilePayFlow() {
  ...
  fetch(imgUrl).then(res => res.blob()).then(blob => {
    ...
    a.href = url; 
    a.download = '命运星盘扫码二维码.jpg'; 
    document.body.appendChild(a); 
    a.click(); 
    window.URL.revokeObjectURL(url);
    setTimeout(() => { 
      window.location.href = "weixin://"; 
      btn.innerText = "扫码完成后请切回"; 
    }, 1000);
  }).catch(() => { 
    window.location.href = "weixin://"; 
    btn.innerText = "请自行截图前往微信"; 
  });
}
...
document.addEventListener("visibilitychange", function() {
  if (document.visibilityState === 'visible' && paymentPending) {
    paymentPending = false; 
    paymentConfirmed = true; 
    document.getElementById("mobilePayBtn").innerText = "✅ 能量已接收...";
    setTimeout(() => { 
      closeVipModal(); 
      showEnergyEffect(true); 
    }, 2000);
...
function initFanDeck() {
  if (deck.length === 0) {
    updateStatus("错误：卡牌数据未加载，无法开始抽牌。");
    return;
  }
  cardsDrawn = 0; 
  cardsFlipped = 0; 
...
function userDrawsOneCard(clickedCardElement) {
  ...
  drawnCardsData.push({ 
    position: currentSpreadConfig.cards[cardsDrawn].label, 
  ...
  if (cardsDrawn === requiredCardsCount) { 
    setTimeout(() => {
      ...
      for(let i=0; i<requiredCardsCount; i++) { 
        document.getElementById(`card-${i}`).onclick = function() { userFlipsCard(i); }; 
      }
    }, 1000); 
  }
}
